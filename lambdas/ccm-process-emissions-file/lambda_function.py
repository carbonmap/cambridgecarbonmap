...
s3 = boto3.client('s3', 'eu-west-2', config=botocore.config.Config(s3={'addressing_style':'path'}))
...
def process_item(conn, id, start, end, measure, unit, value):
    co2e = None
    calc = None
    ...
    with conn.cursor() as cur:
        print('Inserting {}, {}, {}, {}, {}, {}'.format(id, start, end, measure, unit, value))
        cur.execute('insert into readings (reporting_entity, period_start, period_end, measure, unit, value) \
            ...
            [id, start, end, measure, unit, value, unit, value])
        print('Inserting {}, {}, {}, {}, {}, {}, {}, {}'.format(id, start, end, measure, unit, value, co2e, calc))
        cur.execute('insert into emissions (reporting_entity, period_start, period_end, measure, unit, value, kgco2e, co2e_calculation) \
            ...
            [id, start, end, measure, unit, value, co2e, calc, unit, value, co2e, calc])
...
def lambda_handler(event, context):
    ...
    try:
        ...
        print('Got exts: {}'.format(json.dumps(exts)))
        if len(exts) > 0:
            if(exts[0] == '.json'):
